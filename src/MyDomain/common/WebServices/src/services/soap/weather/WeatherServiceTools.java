...
import soap.weather.WeatherStub;
...
public class WeatherServiceTools extends WebServiceTools {
	...
	private void initMainStub(String componentPath) throws AxisFault, ServiceException {
		mStub = new WeatherStub(getWebService().getWebServiceBaseUrl() + getWebServiceName());
	...
	private boolean initOtherStub(String componentPath) throws AxisFault, ServiceException {
		if (getWebService().isComponentOverridden(componentPath)) {
			...
			if (!StringUtils.isEmpty(overriddenWebServiceUrl)) {
				otherStub = new WeatherStub(getWebService().getOverriddenWebServiceUrl(componentPath));
	...
	public GetCityWeatherByZIPResponseDocument getWeatherByZipCode(GetCityWeatherByZIPDocument pReq)
			throws RemoteException {
		logDebug("Processing retrieveDocumentList link Webservice call ....");
		final WeatherStub stub = (WeatherStub) getCurrentStub(this.getAbsoluteName());
		...
		GetCityWeatherByZIPResponseDocument res = null;

		final CacheableXmlObject cacheableReq = new CacheableXmlObject(pReq);

		try {
			res = (GetCityWeatherByZIPResponseDocument) mCachedDocumentListResponses.get(cacheableReq,
					new Callable<GetCityWeatherByZIPResponseDocument>() {
						@Override
						public GetCityWeatherByZIPResponseDocument call() throws RemoteException {
							return stub.getCityWeatherByZIP((GetCityWeatherByZIPDocument) cacheableReq.getXmlObject());
						}
					});
		...
		if (res == null || res.getGetCityWeatherByZIPResponse() == null
				|| res.getGetCityWeatherByZIPResponse().getGetCityWeatherByZIPResult() == null) {
			return null;
		}
	...
	public GetCityWeatherByZIPDocument getGetCityWeatherByZIPDocument() {
		GetCityWeatherByZIPDocument getCityWeatherByZIPDocument = GetCityWeatherByZIPDocument.Factory.newInstance();
		getCityWeatherByZIPDocument.addNewGetCityWeatherByZIP();

		return getCityWeatherByZIPDocument;
	}
}
